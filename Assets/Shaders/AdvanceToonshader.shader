Shader "Cg shader/AdvanceToonshader"
{
	...
	Properties
	{
		...
		// makes diffuse effect on the material
		_DiffuseThreshold("Threshold for Diffuse Colors", Range(0,1)) = 0.1
		// outline colour
		_OutlineColour("Outline Color", Color) = (0,0,0,0)
		// thicc lines
		...
		SubShader
	{
		...
		Pass
		{
			...
			float4 frag(vertexOutput input) : COLOR
			{
				...
				// Low proority: Diffuse illumination
				if (attenuation * max(0.0, dot(normalDirection, lightDirection))
					>= _DiffuseThreshold)
				{
					fragmentColor = _LightColor0.rbg * _Color.rgb;
				}

				// higer priority: outline
